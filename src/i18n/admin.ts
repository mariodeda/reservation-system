/** Admin-only i18n strings — separate from the restaurant site's messages.ts. */
export const am = {
  nav: {
    dashboard: "Dashboard",
    reservations: "Reservations",
    analytics: "Statistics",
    clientsAndStatistics: "Clients & Statistics",
    availability: "Availability",
    customers: "Clients",
    tables: "Tables",
    settings: "Settings",
    docs: "Help",
    signOut: "Sign out",
  },
  status: {
    pending: "Pending",
    confirmed: "Confirmed",
    seated: "Seated",
    completed: "Completed",
    cancelled: "Cancelled",
    no_show: "No-show",
  },
  actions: {
    reinstate: "Reinstate",
    confirmReservation: ["Confirm", "Reservation"],
  },
  reservationOrigin: {
    bookingOrigin: "Booking origin",
    hint: (origin: string) => `Booking originated from ${origin}`,
    google: "Google",
    google_maps: "Google Maps",
    instagram: "Instagram",
    facebook: "Facebook",
    external_other: "Other referral",
  },
  row: {
    guests: "guests",
    manual: "manual",
    theFork: "TheFork",
    externalPlatform: (platform: string) => `External - ${platform}`,
    externalReadOnlyHint: (platform: string) => `Imported from ${platform}. Booking details and status must be managed on ${platform}; table assignment is local and can be changed here.`,
    externalOrigin: (origin: string) => `Origin: ${origin}`,
    externalStatus: (status: string) => `External status: ${status}`,
    externalMealStatus: (status: string) => `External meal status: ${status}`,
    externalUpdatedAt: (time: string) => `External updated: ${time}`,
    theForkReadOnlyHint: "Imported from TheFork. Booking details and status must be managed in TheFork; table assignment is local and can be changed here.",
    edit: "Edit reservation",
    delete: "Delete reservation",
    collapse: "Collapse",
    expand: "Expand",
    table: "Table",
    assignTable: "Assign a table…",
    tablePlaceholder: "e.g. 4, Terrace 2…",
    tableUnassigned: "Unassigned",
    tableSeats: (n: number) => `seats ${n}`,
    tableSuggest: "Suggest",
    tableSuggestNone: "No free table fits this party.",
    tableConflict: "Table is taken at that time.",
    tableReassign: "Click to reassign table",
    tableClear: "Clear table assignment",
    save: "Save",
    saving: "Saving…",
    cancel: "Cancel",
    nameRequired: "Name is required.",
    markedAs: (s: string) => `Marked ${s}`,
    deleteConfirm: (name: string) =>
      `Delete reservation for ${name}? This cannot be undone.`,
    deleteDialogTitle: "Delete reservation?",
    deleteDialogBody: (name: string) =>
      `You are about to permanently delete the reservation for ${name}. This removes it from the calendar and staff views.`,
    deleteDialogWarning: "This is a destructive operation and cannot be undone.",
    deleteDialogAction: "Delete reservation",
    deleted: "Reservation deleted",
    updated: "Reservation updated",
    updateError: "Could not update reservation.",
    deleteError: "Could not delete reservation.",
    saveError: "Could not save changes.",
    tableError: "Could not assign table.",
    // EditForm placeholders
    name: "Name",
    phone: "Phone",
    email: "Email",
    occasion: "Occasion",
    notes: "Notes",
    durationOverride: "Table time",
    durationOverrideDefault: "Default",
  },
  walkIn: {
    button: "Walk-in",
    title: "Walk-in",
    subtitle: "Quick-add — seats now, date is today",
    name: "Guest name *",
    partySize: "Guests",
    table: "Table (optional)",
    add: "Seat walk-in",
    adding: "Seating…",
    added: "Walk-in seated",
    nameRequired: "Guest name is required.",
  },
  reservations: {
    title: "Reservations",
    newReservation: "+ New reservation",
    close: "Close",
    today: "Today",
    goToToday: "Go to today",
    goToTomorrow: "Go to tomorrow",
    covers: "covers",
    exportCsv: "Export CSV",
    print: "Print",
    all: "All",
    noReservationsDay: "No reservations for this day.",
    searchPlaceholder: "Search (2+ chars) — name, email, phone, ref…",
    couldNotLoad: "Could not load reservations.",
    noMatch: (q: string) => `No reservations match "${q}".`,
    results: (n: number, q: string) =>
      `${n} result${n === 1 ? "" : "s"} across all dates for "${q}"`,
    added: "Reservation added",
    couldNotCreate: "Could not create reservation.",
    guestNameRequired: "Guest name is required.",
    customTime: "Custom time (override configured slots)",
    dateLabel: "Date",
    serviceLabel: "Service",
    timeLabel: "Time",
    guestNameLabel: "Guest name",
    notesLabel: "Notes",
    guestNamePlaceholder: "Guest name *",
    notesPlaceholder: "Notes / allergies",
    viewDay: "View day →",
    offeringLabel: "Offering",
    calendarTitle: "Day calendar",
    defaultService: "Dinner",
    add: "Add",
    stopOnlineBooking: "Stop online booking",
    stopOnlineBookingDisabled: "Online booking is already closed for this slot.",
    onlineBookingStopped: "Online booking stopped for this slot.",
    phone: "Phone",
    email: "Email",
    guests: "Guests",
  },
  dashboard: {
    title: "Today",
    allReservations: "All reservations →",
    noReservationsToday: "No reservations today.",
    couldNotLoad: "Could not load today’s reservations.",
    stats: {
      reservations: "Reservations",
      covers: "Covers",
      pending: "Pending",
    },
    booking: (n: number) => `${n} booking${n === 1 ? "" : "s"}`,
    cover: (n: number) => `${n} cover${n === 1 ? "" : "s"}`,
  },
  login: {
    staffSignIn: "Staff sign in",
    operatorSignIn: "Operator sign in",
    username: "Username",
    password: "Password",
    signIn: "Sign in",
    signingIn: "Signing in…",
    loginFailed: "Login failed.",
    networkError: "Network error. Please try again.",
  },
  landing: {
    title: "Staff sign in",
    subtitle: "Enter your restaurant code to continue",
    code: "Restaurant code",
    continue: "Continue",
  },
  theme: {
    toggleLight: "Switch to light mode",
    toggleDark: "Switch to dark mode",
  },
  bookingControls: {
    button: "Today bookings",
    title: "Today booking controls",
    subtitle: "Stop online bookings for a service immediately for today.",
    loading: "Loading services...",
    none: "No services configured for today.",
    stopped: "Stopped",
    accepting: "Accepting",
    stop: "Stop bookings",
    cutoffPassed: "Booking cutoff passed",
    cutoffHint: (time: string) => `Can change until ${time}`,
    activeCount: (n: number) => `${n} stopped`,
    saved: "Booking controls updated",
    saveError: "Could not update booking controls.",
  },
  notifications: {
    title: "Notifications",
    manualConfirmationTitle: "Manual confirmations",
    reconnecting: "Reconnecting...",
    reconnectingShort: "Reconnecting...",
    unreadAria: (n: number) => `Notifications, ${n} unread`,
    manualConfirmationUnreadAria: (n: number) => `Manual confirmations, ${n} unread`,
    markAllRead: "Mark all read",
    noNotifications: "No notifications yet",
    noUnread: "No unread notifications",
    noManualConfirmations: "No manual confirmations",
    noUnreadManualConfirmations: "No unread manual confirmations",
    manualConfirmationHint: "Large-party requests that need tenant approval appear here.",
    listening: "Listening for new reservations...",
    connecting: "Connecting...",
    viewReservations: "View reservations",
    viewAllReservations: "View all reservations",
    dismiss: "Dismiss",
    online: "Online",
    staff: "Staff",
    theFork: "TheFork",
    dish: "DISH",
    externalUpdated: "External update",
    manualReview: "Manual review",
    maxParty: (n: number) => `Max party ${n}`,
    guests: (n: number) => `${n} ${n === 1 ? "guest" : "guests"}`,
    justNow: "just now",
    secondsAgo: (n: number) => `${n}s ago`,
    minutesAgo: (n: number) => `${n}m ago`,
    hoursAgo: (n: number) => `${n}h ago`,
  },
  feedback: {
    send: "Send review email",
    sending: "Sending review email…",
    sent: "Review email sent",
    alreadySent: "Feedback request already sent",
    sendError: "Could not send review email.",
  },
  email: {
    confirmation: "Confirmation",
    feedbackRequest: "Feedback",
    reminder: "Reminder",
    cancellation: "Cancellation",
    sent: "Sent",
    failed: "Failed",
    skipped: "Skipped",
    failedBadge: "Email failed",
    bookingFailedBadge: "Booking email failed",
    bookingFailedAction: "The guest may not have received the booking confirmation. Review the email log and follow up if needed.",
    retryBooking: "Retry booking email",
    retryBookingSending: "Retrying booking email…",
    retryBookingSent: "Booking email sent",
    retryBookingError: "Could not retry booking email.",
    unreachableBadge: "Email unreachable",
    unreachableCallGuest: "Guest email is not reachable. Follow up with a phone call.",
    reasonNoSmtp: "No SMTP configured",
    reasonEventDisabled: "Disabled for this restaurant",
    reasonNoRecipient: "No email address",
    reasonRecipientRejected: "Recipient rejected by SMTP",
    reasonBounced: "Email bounced",
    log: "Email log",
    viewLog: "Email log",
    hideLog: "Hide email log",
    none: "No email activity recorded.",
    loadError: "Could not load email log.",
    attempts: (n: number) => `${n} ${n === 1 ? "attempt" : "attempts"}`,
    sentAt: (when: string) => `Sent ${when}`,
    to: "To",
  },
  analytics: {
    title: "Analytics",
    period7: "7 days",
    period30: "30 days",
    period90: "90 days",
    period365: "1 year",
    covers: "Covers",
    reservations: "Reservations",
    avgParty: "Avg party",
    avgLead: "Avg lead",
    avgLeadUnit: "days",
    byStatus: "By status",
    bySource: "Booking source",
    byService: "By service",
    byOffering: "By offering",
    byDay: "by day",
    newVsReturning: "New vs returning",
    newGuests: "New guests",
    returningGuests: "Returning",
    web: "Online",
    admin: "Staff",
    theFork: "TheFork",
    dish: "DISH",
    noData: "No data for this period.",
    couldNotLoad: "Could not load analytics.",
    noShowRate: "No-show rate",
    cancelRate: "Cancel rate",
    peakDemand: "Peak demand",
    busiestWindow: "Busiest window",
    busiestDay: "Busiest day",
    topSlots: "Top slots",
    peakHint: "Covers by weekday and hour — darker = busier",
    partySizes: "Party sizes",
    partyOf: (n: number) => `${n} ${n === 1 ? "guest" : "guests"}`,
    tableUtilization: "Table utilization",
    tableUtilHint: "Covers and turns per table in this period",
    turns: "turns",
    cov: "cov",
    waitlistTitle: "Waitlist",
    waitlistHint: "Parties added to the waitlist in this period",
    wlTotal: "Added",
    wlSeated: "Seated",
    wlLeft: "Left",
    wlWaiting: "Waiting",
    wlExpired: "Expired",
    wlNeedsAttention: "Needs attention",
    wlConversion: "Seated rate",
    wlAvgWait: "Avg quoted",
    minUnit: "min",
    // KPI card hints
    hintAvgPerDay: (n: string) => `Avg ${n} per day`,
    hintAvgGuestsPerDay: (n: string) => `Avg ${n} guests per day`,
    hintNoBookings: "No bookings in this period",
    hintAvgParty: "avg guests per booking",
    hintAvgLead: "days guests book in advance",
    hintNoShow: "guests who didn't arrive",
    hintCancelRate: "bookings cancelled before visit",
    externalSources: "External booking sources",
    externalSourcesHint: "Imported reservations that affect your availability",
    externalBookings: "External bookings",
    externalCovers: "External covers",
    ofReservations: "of reservations",
    ofCovers: "of covers",
    fromExternalSources: "from external sources",
    cancelled: "Cancelled",
    noShow: "No-show",
    noExternalSources: "No external bookings in this period.",
    onlineBookingOrigins: "Online booking origins",
    onlineBookingOriginsHint: "Attributed internal online bookings only",
    noOnlineOrigins: "No attributed online bookings in this period.",
    attributedOnlineBookings: "Attributed online bookings",
    attributionRate: "Attribution rate",
    // Review request section
    feedbackTitle: "Review requests",
    feedbackNone: "No review requests sent in this period.",
    feedbackRequests: (sent: number) => `${sent} review request${sent === 1 ? "" : "s"} sent`,
  },
  tables: {
    title: "Tables",
    subtitle: "Manage your physical tables — used for floor view and seating.",
    add: "+ Add table",
    label: "Label",
    labelPlaceholder: "e.g. 12, Terrace 3",
    capacity: "Seats",
    minParty: "Min party",
    zone: "Zone / room",
    zonePlaceholder: "e.g. Terrace (optional)",
    offering: "Offering",
    anyOffering: "Any offering",
    joinable: "Joinable",
    joinableHint: "Can be merged with adjacent tables for large parties",
    active: "Active",
    inactive: "Inactive",
    create: "Create table",
    creating: "Creating…",
    save: "Save",
    saving: "Saving…",
    cancel: "Cancel",
    edit: "Edit",
    deactivate: "Deactivate",
    deactivateConfirm: (label: string) =>
      `Deactivate table ${label}? It will drop off pickers but keep its history.`,
    deactivateDialogTitle: "Deactivate table?",
    deactivateDialogWarning: "The table will no longer appear in assignment pickers. Existing reservation history is preserved.",
    deactivated: "Table deactivated",
    created: "Table created",
    updated: "Table updated",
    none: "No tables yet. Add your first table to enable the floor view and seating.",
    labelRequired: "A table label is required.",
    couldNotLoad: "Could not load tables.",
    couldNotSave: "Could not save table.",
    seatsN: (n: number) => `${n} seats`,
    tablesN: (n: number) => `${n} table${n === 1 ? "" : "s"}`,
  },
  floor: {
    title: "Floor",
    free: "Free",
    closed: "Closed",
    reserved: "Reserved",
    seated: "Seated",
    none: "No tables set up — add tables to see the floor.",
  },
  waitlist: {
    title: "Waitlist",
    subtitle: "Same-day queue for guests waiting to be seated. It does not block online booking capacity until you seat the party.",
    flowTitle: "How it works",
    flowAdd: "Add the party when they arrive or call.",
    flowNotify: "Mark notified when you have told them a table is nearly ready.",
    flowSeat: "Seat converts the waitlist entry into a real reservation for this date.",
    activeQueue: "Active queue",
    add: "+ Add to waitlist",
    waitingN: (n: number) => `${n} waiting`,
    none: "No one waiting.",
    name: "Guest name *",
    partySize: "Guests",
    phone: "Phone (optional)",
    pager: "Pager (optional)",
    notes: "Notes (optional)",
    quoted: "Estimated wait",
    quotedMin: (n: number) => `~${n} min`,
    waitingFor: (n: number) => `${n} min`,
    statusWaiting: "Waiting",
    statusNotified: "Notified",
    addBtn: "Add",
    adding: "Adding…",
    added: "Added to waitlist",
    nameRequired: "Guest name is required.",
    notify: "Mark notified",
    notifyHint: "Use after calling, texting, or paging the guest.",
    notified: "Notified",
    seat: "Seat now",
    seatHint: "Creates a reservation and removes this party from the active waitlist.",
    seatFormTitle: "Create reservation from waitlist",
    seating: "Seating…",
    seated: "Seated",
    seatedWithTable: "Seated and table assigned",
    seatedNoTable: "Seated — table not assigned (taken)",
    left: "Mark left",
    leftHint: "Guest left before being seated; keeps the waitlist history.",
    markedLeft: "Removed from waitlist",
    remove: "Remove",
    removeConfirm: (name: string) => `Remove ${name} from the waitlist?`,
    removeDialogTitle: "Remove from waitlist?",
    removeDialogWarning: "This removes the party from the active waitlist. Use Mark left instead when you want to keep that specific outcome.",
    confirm: "Confirm",
    cancel: "Cancel",
    time: "Time",
    service: "Service",
    table: "Table (optional)",
    noTable: "No table",
    couldNotSave: "Could not update the waitlist.",
  },
  settings: {
    title: "Settings",
    changePassword: "Change password",
    currentPassword: "Current password",
    newPassword: "New password",
    confirmPassword: "Confirm new password",
    updatePassword: "Update password",
    saving: "Saving…",
    passwordTooShort: "New password must be at least 8 characters.",
    passwordMismatch: "Passwords do not match.",
    passwordUpdated: "Password updated successfully.",
    passwordError: "Could not update password.",
    emailPreferences: "Email preferences",
    feedbackAutoSend: "Automatically send review emails",
    feedbackAutoSendDescription: "Send the review request automatically after a completed reservation once the tenant delay has elapsed.",
    feedbackAutoSendHint: "Feedback request emails are enabled by the platform. Staff can decide whether completed reservations should trigger them automatically.",
    feedbackAutoSendDisabledHint: "Feedback request emails are disabled by the platform. This option is unavailable until they are enabled for this restaurant.",
    emailPreferencesSaved: "Email preferences saved.",
    emailPreferencesError: "Could not update email preferences.",
    capacityPreferences: "Capacity preferences",
    capacityModeTables: "Use table capacity",
    capacityModeTablesDescription: "Online availability is calculated from active tables and table duration.",
    capacityModeManual: "Use manual slot capacity",
    capacityModeManualDescription: "Staff set default covers per service and can override individual slots.",
    capacityTablesHint: "Capacity controls are based on the Tables page. Manual slot-capacity fields stay hidden.",
    capacityManualHint: "Manual capacity is active. Weekly hours define baseline slot covers, and reservations can override slots.",
    capacityPreferencesSaved: "Capacity preferences saved.",
    capacityPreferencesError: "Could not update capacity preferences.",
  },
  availability: {
    title: "Availability",
    saveChanges: "Save changes",
    saving: "Saving…",
    saved: "Availability saved",
    savedIndicator: "✓ Saved",
    unsavedIndicator: "Unsaved changes",
    unsavedLeaveWarning: "You have unsaved availability changes. Leave without saving?",
    couldNotLoad: "Could not load configuration.",
    couldNotSave: "Could not save changes.",
    loading: "Loading…",
    // Booking rules section
    bookingRules: "Booking rules",
    minParty: "Min party",
    maxParty: "Max party",
    bookingWindow: "Booking window (days)",
    leadTime: "Lead time (min)",
    leadHint: "Lead time = guests can't book a slot starting within this many minutes from now.",
    // Weekly schedule
    weeklyHours: "Weekly hours",
    editingPrefix: "Editing",
    open: "Open",
    closed: "Closed",
    none: "None.",
    remove: "Remove",
    // Service window editor
    addService: "+ Add service",
    serviceName: "Service name",
    serviceNameHint: "Label shown to staff and guests for this bookable service.",
    serviceFrom: "From",
    serviceFromHint: "First bookable time for this service, using 24-hour time.",
    serviceTo: "To",
    serviceToHint: "Last bookable time generated for this service, using 24-hour time.",
    serviceInterval: "Every (min)",
    serviceIntervalHint: "Minutes between generated booking slots.",
    serviceCapacity: "Default covers",
    serviceCapacityHint: "Baseline maximum covers for each generated slot in this service when manual capacity is enabled.",
    // Special dates
    specialDates: "Special dates",
    specialDatesHint: "One-off hours for a single date (holidays, special events) — these replace the normal weekly hours for that day only.",
    dateAlreadyExists: "That date already has special hours.",
    addSpecialDate: "Add special date",
    // Closed days
    closedDays: "Closed days",
    closedDaysHint: "Holidays / days off. These dates are fully blocked on the public site.",
    addClosedDay: "Add closed day",
    // Blocked slots
    blockedSlotsTitle: "Blocked time slots",
    blockedSlotsHint: "Block individual times on a specific date (e.g. a private event) without closing the whole day.",
    blockedSlotTime: "Time",
    blockSlot: "Block slot",
    // Offerings management
    offerings: "Offerings",
    offeringsHint: "This venue takes bookings for one offering. Add another (e.g. Cocktails, Events) to manage its schedule and capacity separately.",
    offeringsHintEmpty: "Add an offering to start taking bookings.",
    addOffering: "+ Add offering",
    removeOffering: "Remove offering",
    removeOfferingConfirm: (label: string) =>
      `Remove the "${label}" offering? Its schedule will be deleted. Existing reservations are kept.`,
    removeOfferingDialogTitle: "Remove offering?",
    removeOfferingDialogWarning: "This deletes the offering schedule from availability. Existing reservations are not deleted.",
    offeringName: "Name",
    // Table duration
    tableDuration: "Default table duration",
    tableDurationHint: "Fallback duration used when a service leaves Duration on default. It drives table conflicts, public availability, and booking capacity.",
    serviceDuration: "Duration",
    serviceDurationHint: "Overrides the default table duration for this service and day. Leave on default to inherit the value above.",
    actions: "Actions",
    actionsHint: "Service-row controls, such as removing this service window.",
    durDefault: "System default (2h)",
    durInherit: "Global default",
    durLabel: (min: number) => {
      const h = Math.floor(min / 60);
      const m = min % 60;
      return m ? `${h}h ${m}min` : `${h}h`;
    },
    // DayOccupancy inline messages
    dayError: "Could not load availability for this date.",
    dayClosed: "Closed on this date.",
    dayNoService: "No service configured for this date.",
    tableCapacityBanner: "Covers capacity is currently dictated by active tables. Manual service and slot capacity controls are hidden.",
    manageTables: "Manage tables",
    fullyBooked: "Fully booked",
    slotUnavailable: "Unavailable (past, too soon, or blocked)",
    slotServiceStopped: "Service stopped today",
    slotBlocked: "Time blocked",
    slotActionsTitle: "Slot actions",
    slotCoversLabel: "Covers",
    slotRemainingLabel: (remaining: number) => `${remaining} left online`,
    slotOnlineLabel: "Online bookings",
    slotOnlineOpen: "Open",
    slotOnlineClosedCapacity: "Closed - over capacity",
    slotOnlineStopped: "Stopped for guests",
    slotStaffActions: "Staff actions",
    slotPublicBookingActions: "Guest booking controls",
    slotCapacityActions: "Manual capacity",
    slotCapacityLabel: "Maximum covers for this slot",
    slotCapacityEditHint: "Adjust the guest-facing capacity for this exact service and time.",
    slotCapacityToday: "Today only",
    slotCapacityTodayHint: "Override this slot only for the selected date.",
    slotCapacityFuture: "All days moving forward",
    slotCapacityFutureHint: "Apply to this same service and time from this date onward.",
    slotCapacitySave: "Save slot capacity",
    slotCapacitySaved: "Slot capacity updated.",
    slotCapacityError: "Could not update this slot capacity.",
    slotAddReservationAction: (time: string) => `Add reservation at ${time}`,
    slotStopAction: (time: string) => `Stop online bookings for ${time}`,
    slotResumeAction: (time: string) => `Resume online bookings for ${time}`,
    slotStopHint: "Guests will see only this exact time as unavailable online.",
    slotResumeHint: "Guests can book this exact time again if covers are available.",
    slotStopSaved: "Slot booking state updated.",
    slotStopError: "Could not update this slot.",
    slotLeadTime: "Booking cutoff passed",
    slotNotEnoughCovers: "Not enough covers left",
    serviceEnded: "Service ended",
    openShort: "Open",
    lowShort: "Low",
    criticalShort: "Critical",
    fullShort: "Full",
    unavailableShort: "Unavailable",
    overbookedShort: (n: number) => `Over by ${n}`,
    overbookedStateShort: "Overbooked",
    overbookedTitle: (n: number) => `Overbooked by ${n} ${n === 1 ? "cover" : "covers"}`,
    overbookedByLabel: (n: number) => `${n} over configured capacity`,
    overbookedHint: "External or staff bookings have exceeded the configured table capacity. Public availability is closed for this slot, but staff should rebalance tables or contact guests if needed.",
    slotCapacityHelpTitle: "How slot capacity is calculated",
    slotCapacityHelpIntro: "Each slot shows covers already using capacity during a new booking's table-duration window, not only reservations that start at that exact time.",
    slotCapacityHelpExample: "Example: with a 2-hour table duration, a new 18:30 booking uses 18:30-20:30. It overlaps existing 19:00 and 20:00 reservations, so both count in the 18:30 slot.",
    slotCapacityHelpReason: "This prevents accepting bookings that would fit at the start time but collide before the table would be free.",
    slotCapacityHelpClose: "Got it",
    slotStatus: (booked: number, capacity: number, remaining: number) =>
      `${booked}/${capacity} covers in turn window · ${remaining} left online`,
    slotOverbookedStatus: (booked: number, capacity: number, overbookedBy: number) =>
      `${booked}/${capacity} covers in turn window - ${overbookedBy} over capacity`,
    covers: (booked: number, capacity: number) => `${booked}/${capacity} in turn window`,
    coverSummaryHint: (booked: number, capacity: number) =>
      `${booked}/${capacity} shows the peak booked covers during this service compared with active table seats. Time slots count overlapping reservations using the configured table duration.`,
    coversAvailable: (available: number, capacity: number, pct: number) =>
      `${available}/${capacity} covers available (${pct}%)`,
    coversAvailableOk: "Healthy availability",
    coversAvailableLow: "Low availability: fewer than 30% of covers are still bookable.",
    coversAvailableCritical: "Critical availability: fewer than 5% of covers are still bookable.",
  },
  print: {
    subtitle: "Service run sheet",
    headerStats: (reservations: number, covers: number) =>
      `${reservations} covers — ${covers} guests`,
    vips: "★ VIPs:",
    dietary: "⚠ Dietary alerts:",
    print: "Print",
    close: "Close",
    loading: "Loading…",
    noReservations: "No reservations for this day.",
    serviceStats: (bookings: number, guests: number) =>
      `${bookings} bookings · ${guests} guests`,
    colTime: "Time",
    colGuests: "Guests",
    colName: "Name",
    colTable: "Table",
    colPhone: "Phone",
    colStatus: "Status",
    colNotes: "Notes / Dietary",
    printed: "Printed:",
  },
  platform: {
    title: "Restaurants",
    newRestaurant: "New restaurant",
    close: "Close",
    loading: "Loading…",
    noRestaurants: "No restaurants yet. Create one to get started.",
    noHostsMapped: "no hosts mapped",
    couldNotLoad: "Could not load restaurants.",
    statusActive: "active",
    statusDisabled: "disabled",
    totalShort: "total",
    last30Short: "/30d",
    noShows: (n: number) => `${n} no-show${n === 1 ? "" : "s"}`,
    noShowsTitle: "No-shows",
    lastBooking: (date: string) => `Last: ${date}`,
    lastBookingTitle: "Last booking date",
    emailSummary: "Email",
    emailGloballyOff: "All email off",
    emailConfirmation: "Confirmations",
    emailFeedback: "Feedback",
    emailOn: "On",
    emailOff: "Off",
    syncSummary: "External sync",
    syncActive: "Sync active",
    syncInactive: "Sync off",
    syncNotConfigured: "Not configured",
    syncLastRun: "Last sync",
    syncLastError: "Last error",
    smtpStatus: "SMTP",
    smtpOk: "Connected",
    smtpFailed: "Failed",
    smtpUnknown: "Not checked",
    smtpNotConfigured: "Not configured",
    smtpCheckNow: "Check SMTP",
    smtpChecking: "Checking SMTP…",
    smtpCheckFailed: "Could not check SMTP.",
    smtpCheckComplete: (n: number) => `SMTP checked for ${n} restaurant${n === 1 ? "" : "s"}.`,
    totals: {
      bookings: "Total bookings",
      last30: "Last 30 days",
      active: "Active restaurants",
    },
    create: {
      title: "New restaurant",
      name: "Name",
      slug: "Slug",
      username: "Staff username",
      password: "Staff password (8+)",
      hosts: "Hosts (space/comma separated)",
      create: "Create restaurant",
      creating: "Creating…",
      created: "Restaurant created",
      validationError: "Name, slug, username and an 8+ char password are required.",
      error: "Could not create restaurant.",
    },
    tenant: {
      couldNotLoad: "Could not load restaurant.",
      couldNotSave: "Could not save.",
      requestFailed: "Request failed",
      saved: "Settings saved",
      saveSettings: "Save settings",
      saving: "Saving…",
      impersonate: "Impersonate",
      couldNotImpersonate: "Could not start impersonation.",
      disable: "Disable",
      enable: "Enable",
      delete: "Delete",
      deleted: "Restaurant deleted",
      couldNotDelete: "Could not delete.",
      disableConfirm: (name: string) => `Disable ${name}? Guests will no longer be able to book until re-enabled.`,
      enableConfirm: (name: string) => `Enable ${name}? Bookings will resume immediately.`,
      deleteConfirm: (name: string) => `Delete ${name}? This removes all reservations and cannot be undone.`,
      disableDialogTitle: "Disable restaurant?",
      enableDialogTitle: "Enable restaurant?",
      removeHostDialogTitle: "Remove host?",
      deleteDialogTitle: "Delete restaurant?",
      deleteDialogWarning: "This is destructive and requires platform password confirmation.",
      clearDialogTitle: "Clear tenant data?",
      clearDialogWarning: "This deletes generated operational data for this tenant and cannot be undone.",
      firstTheForkSyncTitle: "Run first TheFork sync?",
      firstTheForkSyncBody: "This imports upcoming TheFork reservations through the tenant booking window without sending tenant popup notifications.",
      dishHistorySyncTitle: "Sync DISH history?",
      dishHistorySyncBody: "This imports DISH reservations from the last 60 days. Existing imports will be skipped and tenant popup notifications will not be sent.",
      // Sections
      identity: "Identity & branding",
      bookingApi: "Booking API",
      emailFlow: "Email flow",
      smtp: "Email (SMTP)",
      emailTemplates: "Email templates",
      hostsSection: "Hosts",
      staffLogin: "Staff login",
      debug: "Debug — mock data",
      // Identity fields
      displayName: "Display name",
      publicUrl: "Public URL",
      reviewUrl: "Review site URL",
      contactEmail: "Contact email",
      contactPhone: "Contact phone",
      locale: "Locale",
      timezone: "Timezone",
      themePrimary: "Theme primary (#hex)",
      themeOnPrimary: "Theme on-primary (#hex)",
      logoUrl: "Logo URL (login + admin header)",
      autoConfirm: "Auto-confirm web bookings",
      // Booking API
      publicKey: "Public tenant key",
      allowedOrigins: "Allowed origins (one per line — scheme://host[:port])",
      keyCopied: "Key copied",
      copy: "Copy",
      presetLoaded: (name: string) => `Loaded "${name}"`,
      statusUpdated: (status: string) => `Restaurant ${status}`,
      platformPasswordPrompt: "Confirm with your platform password",
      operationDone: (label: string) => `${label} done`,
      // Email flow
      emailEnabled: "Enable all outbound email for this tenant",
      emailBookingConfirmation: "Booking confirmation email",
      emailFeedbackRequest: "Post-visit review request email",
      emailReservationReminder: "Pre-visit reminder email",
      emailCancellationConfirmation: "Cancellation email",
      feedbackDelay: "Review request delay (hours after reservation)",
      reminderLead: "Reminder lead time (hours before reservation)",
      bookingApiHint: "A marketing site books against the shared reservation service by setting its NEXT_PUBLIC_RESERVATIONS_TENANT to this public key, and is allowed to call the API only from the origins listed below.",
      emailFlowHint: "Platform-only controls for this restaurant's outbound email automation. Staff cannot edit these from the tenant admin.",
      emailEnabledHint: "When off, every event below is suppressed even if SMTP credentials and templates are configured.",
      confirmationHint: "Sent after confirmed public bookings when auto-confirm is on, or after any confirmed booking path calls the confirmation event.",
      cancellationHint: "Sent only when a guest cancels online or staff cancels a tenant-owned reservation.",
      // SMTP
      smtpHost: "SMTP host",
      smtpPort: "Port",
      smtpUser: "Username",
      smtpFrom: "From (optional)",
      smtpPassword: "Password",
      smtpPassSet: "(set — leave blank to keep)",
      smtpSecure: "Use TLS (secure)",
      // Email templates
      bookingConfirmationTpl: "Booking confirmation",
      feedbackRequestTpl: "Feedback request",
      reminderTpl: "Reminder",
      cancellationTpl: "Cancellation",
      calendarEventTitle: "Calendar event title",
      calendarEventTitleHint: "The booking confirmation calendar attachment uses the calendar event title; blank means \"{{restaurantName}} reservation\".",
      subject: "Subject",
      plainText: "Plain text body",
      htmlBody: "HTML body",
      presets: "Presets:",
      preview: "Preview",
      templatesHint: "Leave blank to use the platform default. Variables:",
      feedbackTemplateHint: "Feedback template also accepts",
      previewTitle: "Email preview",
      previewSample: "sample data",
      closePreview: "Close preview",
      // Hosts
      addHost: "Add host",
      hostMapped: "Host mapped",
      couldNotMapHost: "Could not map host.",
      removeHostConfirm: (host: string, name: string) => `Remove ${host} from ${name}? Traffic via this domain will stop routing to this restaurant.`,
      noHosts: "None mapped.",
      hostsHint: "DNS-alias these hostnames to this deployment. Requests on them resolve to this restaurant.",
      // Staff login
      newPassword: "New password (8+ chars)",
      setPassword: "Set password",
      passwordUpdated: "Staff password updated",
      passwordTooShort: "Password must be at least 8 characters.",
      couldNotSetPassword: "Could not set password.",
      staffLoginHint: (slug: string) => `Reset the password staff use at this restaurant's /admin/${slug}/login.`,
      // Debug / mock
      generateAll: "Generate everything",
      mockTables: "Tables (floor plan)",
      mockToday: "Reservations · today",
      mockUpcoming: "Reservations · upcoming",
      mockHistory: "Reservations · history",
      mockWaitlist: "Waitlist",
      mockCustomers: "Customers (VIP/dietary)",
      mockFeedback: "Review requests",
      clearData: "Clear all data",
      clearing: "Clearing…",
      working: "Working…",
      clearConfirm: (name: string) => `Delete ALL reservations, tables, waitlist, customers and feedback for ${name}? This cannot be undone.`,
      debugHint: (name: string) => `Generate realistic test data for ${name} to exercise every admin screen. Generators are additive; use "Clear all data" to reset. For test tenants only.`,
      networkError: "Network error.",
      operationFailed: "Operation failed.",
    },
  },
  customers: {
    title: "Customers",
    searchPlaceholder: "Search by name, email, or phone…",
    noResults: "No customers found.",
    couldNotLoad: "Could not load customers.",
    couldNotSave: "Could not save profile.",
    saved: "Profile saved",
    vip: "VIP",
    vipToggle: "Mark as VIP",
    dietaryNotes: "Dietary & allergies",
    dietaryPlaceholder: "e.g. nut allergy, gluten-free, vegan…",
    staffNotes: "Staff notes",
    staffNotesPlaceholder: "Internal notes — preferences, incidents, anything useful for the team…",
    save: "Save profile",
    saving: "Saving…",
    cancel: "Cancel",
    visitHistory: "Visit history",
    noHistory: "No reservations on record.",
    firstVisit: "First visit",
    lastVisit: "Last visit",
    totalVisits: "Visits",
    totalCovers: "Covers",
    sortLabel: "Sort",
    sortLastVisit: "Recent first",
    sortName: "Name A–Z",
    sortVisits: "Most visits",
    exportCsv: "Export CSV",
    nthVisit: (n: number) => {
      const mod10 = n % 10, mod100 = n % 100;
      const s = mod100 >= 11 && mod100 <= 13 ? "th"
        : mod10 === 1 ? "st" : mod10 === 2 ? "nd" : mod10 === 3 ? "rd" : "th";
      return `${n}${s} visit`;
    },
    customerCount: (n: number) => `${n} customer${n === 1 ? "" : "s"}`,
    dietaryAlert: "Dietary notes on file",
    noShows: "No-shows",
    cancellations: "Cancellations",
    reliability: "Reliability",
  },
};
